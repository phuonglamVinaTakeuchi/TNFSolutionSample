...
{
    public class CrushedAndPlatformBeamDraw : BeamDrawingBase
    {
        public CrushedAndPlatformBeamDraw(BeamSectionGBase beamGeometry) : base(beamGeometry)
        {
        }
        ...
        private void CreateRevonationPLine(List<Entity> entities)
        {
            ...
            entities.Add(Helper.CreateLine(IntersectBetwenStoneAndBeam(), platformBeam.TopRightThirdRevonation.TopRight, Helper.SECTION_LINE_LAYER_NAME));
        }
        private void CreateHatch(List<Entity> entities)
        {
            ...
            var stoneHatch = new List<Point2D>() {
            platformBeam.Beam.TopRight,
            platformBeam.Floor.BottomRight,
            platformBeam.TopRightThirdRevonation.TopRight,
            IntersectBetwenStoneAndBeam()};
            entities.Add(Helper.CreateHatch(stoneHatch, null, Helper.STONE_HATCH_NAME, StoneHatchScale));

        }
        public Point2D IntersectBetwenStoneAndBeam()
        {
            var platformBeam = BeamGeometry as PlatformBeamSectionG;
            var segment1 = new Segment2D(platformBeam.Beam.BotomRight, platformBeam.Beam.TopRight);
            var segment2 = new Segment2D(platformBeam.ThirdRevonation.TopLeft, platformBeam.ThirdRevonation.TopRight);
            Segment2D.IntersectionLine(segment1, segment2, out var intersectPoint);
